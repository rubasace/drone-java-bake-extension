...
if [ -z "$JDK_JAVA_OPTIONS" ]; then
  ...
  export JDK_JAVA_OPTIONS="-XX:+UseContainerSupport -XX:InitialRAMPercentage=${INITIAL_RAM_PERCENTAGE} -XX:MaxRAMPercentage=${MAX_RAM_PERCENTAGE} -XX:MaxJavaStackTraceDepth=${MAX_JAVA_STACKTRACE_DEPTH} ${EXTRA_JAVA_OPTIONS}"
fi
